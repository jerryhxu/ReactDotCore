...

{
	class Program
	{
		...
		public static async Task MainAsync()
		{
			...
			string hash = "160B2087-9AD2-45B9-8471-33F460823280_0L5FZ52115QVN0Z4N";
			string normalizedHash = hash.ToUpper();
			var config = new GetItemOperationConfig();
			var result = await table.GetItemAsync(
		...
		public static void Main(string[] args)
		{
			...
			serviceCollection.Configure<SQSConfiguration>(Configuration.GetSection("SQS"));

			// create service provider
			var serviceProvider = serviceCollection.BuildServiceProvider();
			SQSConfiguration sqsOptions = serviceProvider.GetService<IOptions<SQSConfiguration>>().Value;
		...
		private static void ConfigureServices(IServiceCollection serviceCollection)
		{
			...
			serviceCollection.AddSingleton<IDynamoDbUniqueConstraintRepository>(provider =>
				{
					var dynamoDbClient = provider.GetService<IDynamoDBClientProvider>();
					var logger = provider.GetService<ILogger<DynamoDbUniqueConstraintRepository>>();
					var tableWrapper = provider.GetService<ITableWrapper>();
		...
		private static void CallDynamoDB()
		{
			...
			string hash = "160B2087-9AD2-45B9-8471-33F460823280_0L5FZ52115QVN0Z4N";
			string normalizedHash = hash.ToUpper();
			var config = new GetItemOperationConfig();			//var result = await table.GetItemAsync(
			//		new Dictionary<string, DynamoDBEntry>
			//		{
			//			{ "Hash", normalizedHash },
			//		},
